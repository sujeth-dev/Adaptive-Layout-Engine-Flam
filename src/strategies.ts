...
import { GAP, inContentOrder } from "./measure";
 ...
 */
function layoutColumn(items: MeasuredElement[], columnRect: Rect, align: "start" | "center"): ResolvedBox[] {
...
function horizontalBand(items: MeasuredElement[], rect: Rect): LayoutCandidate | null {
  ...
  for (let i = 0; i < groups.length; i++) {
    ...
    boxes.push(...layoutColumn(groups[i]!, colRect, "center"));
    x += w + GAP;
  }
...
function sideBySideSplit(items: MeasuredElement[], rect: Rect): LayoutCandidate | null {
  ...
  const boxes: ResolvedBox[] = [];
  if (heroItems.length) {
    boxes.push(...layoutColumn(heroItems, { x: rect.x, y: rect.y, width: heroWidth, height: rect.height }, "center"));
  }
  if (restItems.length) {
    boxes.push(
...
function adaptiveGrid(items: MeasuredElement[], rect: Rect): LayoutCandidate | null {
  const ordered = inContentOrder(items);
  if (ordered.length === 0) return null;
  ...
  const boxes: ResolvedBox[] = ordered.map((item, idx) => {
    ...
    return layoutColumn([item], cellRect, "center")[0]!;
  });

  return { strategy: "adaptive-grid", boxes };
}

export const STRATEGIES: Array<(items: MeasuredElement[], rect: Rect) => LayoutCandidate | null> = [
  verticalStack,
  horizontalBand,
  sideBySideSplit,
  adaptiveGrid,
